...
object Courier {
  ...
  def couriers(courierCount: Int): List[Courier] = Range(1, courierCount).map(i => Courier(i)).toList
...
object CouriersWithComprehension extends App {
  ...
  def serveAddresses(addresses: List[Address], couriers: List[Courier]): List[Address] = {
    val addrIterator = addresses.iterator

    couriers.flatMap(courier => {
      (0 to courier.canServe)
        .withFilter(i => traffic().degree < 5 && addrIterator.hasNext)
        .map(i => addrIterator.next())
    }).toList
  }
